...
) : IRequestHandler<SeedDatabaseCommand>
{
    ...
    {
        if (
            await this.DatabaseManager.CanConnectAsync(cancellationToken)
        ...
        var admin = await this.LeaderRepository.CreateLeader(
            request.Email,
            request.FirstName,
            request.LastName,
            request.Password,
        ...
        var adminGroup = await this.LeaderRepository.CreateGroup(
            "Admins",
            GroupDesignation.Admins,
            null,
            null,
            cancellationToken
        );
        ...
        var leaderGroup = await this.LeaderRepository.CreateGroup(
            "Leitende",
            GroupDesignation.Leitende,
            null,
            null,
            cancellationToken
        );
