...
export const login = async (email: string, password: string): Promise<{ token: string | null; error: string | null }> => {
  try {
    ...
    const response = await axios.post("https://dev-api.stamm-phoenix.de/auth/login", creds);
    ...
    if (axios.isAxiosError(error)) {
      return { token: null, error: `An error occurred while logging in: ${error.message}` };
    } else {
...
export const checkToken = async (token: string): Promise<boolean> => {
  try {
    const response = await axios.get("https://dev-api.stamm-phoenix.de/auth/check", {
      headers: {
